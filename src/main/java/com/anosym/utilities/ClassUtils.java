...
public class ClassUtils {
  ...
  /**
   * Find the field, declared, or in superclass annotated with the specified annotation. If no field
  ...
  public static <T> String getField(Class<T> clazz, Class<? extends Annotation> annot) {
    try {
      ...
      /**
       * if we reach here, we did not get the annotation in the current class, try superclass.
       */
   ...
   * @return
   */
  public static <T> Field getAnnotatedField(Class<T> clazz, Class<? extends Annotation> annot) {
    try {
      Field[] fields = clazz.getDeclaredFields();
      for (Field f : fields) {
        if (f.isAnnotationPresent(annot)) {
      ...
      return getAnnotatedField(clazz.getSuperclass(), annot);
    } catch (Exception e) {
      throw new RuntimeException(e);
    }
  }
  ...
  /**
   * Find the field value, declared, or in superclass annotated with the specified annotation. If no
   * field is found returns null.
   *
   * @param <T>
   * @param o
   * @param annot
